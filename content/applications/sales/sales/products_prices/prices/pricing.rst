...
Settings`. Scroll to the :guilabel:`Pricing` section, and click the checkbox next to the
:guilabel:`Pricelists` feature.
...
apply to in the :guilabel:`Company` field. If left blank, the pricelist is automatically applied to
all companies in the database.

.. note::
   The :guilabel:`Price Rules` tab functionality on a pricelist form varies, depending on the
   :guilabel:`Pricelists` setting chosen, but the :guilabel:`Time-based rules` tab and
   :guilabel:`Configuration` tab are the same, regardless of the chosen :guilabel:`Pricelists`
   setting.
...
.. seealso::
   - :doc:`/applications/sales/sales/products_prices/prices/pricing/multiple-prices-per-product`
   - :doc:`/applications/sales/sales/products_prices/prices/pricing#advanced-price-rules`
...
a drop-down menu of pre-designated time intervals (e.g. `Montly`, `Quartlery`, `Weekly`, etc.).

New time periods can also be directly created from this column, by typing in the new name for the
:guilabel:`Period` and then selecting :guilabel:`Create` to create the time period, which can be
edited later. Or, select :guilabel:`Create and edit...` to reveal a pop-up form, in which the new
time period can be directly configured.
...
With Odoo *Sales*, the option to show the public price *and* the computed discount percentage on
printed sales orders and in the *eCommerce* product catalog is available.
...
either by clicking the :guilabel:`Pricelists` link from the *Sales* :guilabel:`Settings` page, or by
...
a new :guilabel:`Discounts` section is now available.
...
provides the oppportunity to directly apply a different pricelist to customers on their contact
form.
...
:guilabel:`Customers` page, or by clicking on the customer's name on a sales order or quotation.
Either route reveals that customer's contact form.
...
Next, navigate to the :menuselection:`Sales app --> Products --> Products` and search for (or
select) the product for which multiple prices should be applied. Selecting a product from the
:guilabel:`Products` page reveals that specific product's product form on a separate page.
...
.. example::
   ...
   $100, customersccan be encouraged to buy more, if the :guilabel:`Price` is set at $85 per product
...
The :guilabel:`Advanced price rules (discounts, forumlas)` pricelist option provides the option to
...
Either option reveals a separate :guilabel:`Pricelists` page, in which pricelists can be created
...
.. note::
   Each option reveals its own computation-specific fields on the form.
...
.. example::
   To formulate a 100% markup (or 2 times the cost), with a $5 minimum margin, set the
   :guilabel:`Discount` to `-100` and the :guilabel:`Margins` to `5`. This is often seen in retail
   situations.
...
Then, in the :guilabel:`Extra Fee` field, specify a fixed amount to add or subtract (if negative)
to the amount calculated with the discount. After that, enter a desired figure in the
:guilabel:`Rounding Method` field. The rounding method sets the price so that it is a multiple of
...
.. example::
   Here's how it would look to apply 20% discounts, with prices rounded up to 9.99.
...
.. note::
   ...
   takes the rule of the product itself. Make sure at least one pricelist item covers all products.
